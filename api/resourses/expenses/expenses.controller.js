const addExpenses = async (expenses, userID,creditInfo) => {
  ...
  try {
    ...
    if (simulation.length > 0) {
      await query("update simulation set simulation_amount = ? , quotes = ? , commision_id = ? , viatic = ? , created_at = now(), imported = ? where id =?"
      ...
      await query("insert into simulation (simulation_amount,quotes,commision_id,viatic,imported,updated_at,user_id) values (?,?,?,?,?,now(),?)"
    ...
    expenses.map(async item=>{
      ...
      const sql = "insert into expenses (description, amount,tax_id,client_id) values (?,?,?,?)"
...
const addMultipleExpensesPayment = async (pagos,taxes,userId,USER_ID,creditId) => {
  ...
  try {
   ...
   if (creditId) {
     const sql = `insert into cash_flow (type,amount,created_at,description,operation_type,account_id,caja_id,user,responsable_id,credit_id) values (2,?,now(),?,"gasto_otorgamiento",?,1,?,?,?)`
     ...
     const sql = `insert into cash_flow (type,amount,created_at,description,operation_type,account_id,caja_id,user,responsable_id,expense_user_id) values (2,?,now(),?,"gasto_otorgamiento",?,1,?,?,?)`
       insertPaymentArray.map(async payedItem =>{
...
const updateExpenses =async (payedArray,userId,creditId) => {
  ...
  if(creditId){
    payedArray.map(async tax =>{
      await query("update expenses set payed=?, credit_id = ? where id = ?",[tax.pagado,creditId,tax.id])
     })
  }else{
    payedArray.map(async tax =>{
      await query("update expenses set payed=? where credit_id is null and client_id = ? and tax_id = ?",[tax.pagado,userId,tax.id])
     })
  }
  return
}
  ...
  try {
    ...
    if(getExpenses.length > 0) {
      getExpenses.map(async item=>{
          ...
          const sql = "update expenses set credit_id = ? where id = ? "
    ...
    /*                              updateSimulation                              */
    /* -------------------------------------------------------------------------- */
    const getSimulation = await query("select * from simulation where user_id = ?",[userId])
    if(getSimulation.length > 0) {
      const sqlSimulation = "update simulation set simulation_amount = null,quotes = null, commision_id = null, viatic = null, imported = 1, updated_at=now() where user_id = ? "
      await query(sqlSimulation,[userId])
    }
    /* -------------------------------------------------------------------------- */
    /*                               updateCashFlow                               */
    /* -------------------------------------------------------------------------- */
    const getCashFlow = await query(`select * from cash_flow where expense_user_id=? and operation_type="gasto_otorgamiento" and credit_id is null`,[userId])
    if (getCashFlow.length>0) {
      const getIds = getCashFlow.map(item=>item.id)
      await query(`update cash_flow set credit_id =? where id in(?)`,[creditId,getIds])
    }
    return
  } catch (error) {
    return error
...
const getExpenses = (userId) => {
  ...
  const query = util.promisify(mysqli.query).bind(mysqli);
  try {
    const response = query ("select * from expenses where client_id = ? and credit_id is null",[userId])
    return response
  } catch (error) {
    return error
  }
